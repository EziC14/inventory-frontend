<template>
  ...
  <q-page class="bg-page q-gutter-md q-pa-md" style="padding: 100px;">
    <q-card class="shadow-0 bg-page">
      <Title name="Nuevo Producto"> </Title>
    </q-card>
    <q-stepper
      v-model="step"
      ref="stepper"
  ...
  methods: {
    ...
    onCreate() {
      ...
      if (this.product.isProductFinal) {
        this.$store
          .dispatch("general/setFinalProduct", payload, { root: true })
          .then((response) => {
        ...
        this.$store
        .dispatch("general/setPrimaryProduct", payload ,{ root: true })
        .then((response) => {
          if (response.data.status === "OK") {
            this.alertSuccess(response.data.msg);
          }
