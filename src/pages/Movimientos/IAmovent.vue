<template>
  <q-page v-if="showUploadForm" class="flex flex-center" style="background: linear-gradient(135deg, #f5f7fa 0%, #c3cfe2 100%); min-height: 100vh;">
    <q-card style="width: 500px; max-width: 95vw; border-radius: 12px; box-shadow: 0 8px 20px rgba(0,0,0,0.1);">
        <q-card-section class="row items-center q-pa-md bg-primary text-white" style="border-radius: 12px 12px 0 0;">
          <div class="text-h6">Subir Documento</div>
          <q-space />
        </q-card-section>

        <q-card-section class="q-pt-lg q-px-md">
          <div class="q-mb-md">
            <p class="text-body1">Sube un documento para procesamiento inteligente de inventario</p>
          </div>
        ...
        <q-card-actions align="right" class="q-pa-md bg-grey-1" style="border-radius: 0 0 12px 12px;">
          <q-btn
            flat
            label="Cancelar"
            color="negative"
          ...
          <q-btn
            ...
            color="primary"
            :disable="!documentFile || !movementDirection"
...
<script>
    ...
    export default {
      ...
      setup() {
        ...
        const movementDirection = ref(null)
        const showUploadForm = ref(true)
        const processing = ref(false)
        return { $q, router, documentFile, movementDirection, showUploadForm, processing }
      },
      data() {
        return {
          currentStep: 1,
      ...
      computed: {
        verifyFirstStep() {
          const requiresClient =
            this.movement.direction?.value === 'OUT' && !this.movement.client_name
      ...
      methods: {
        ...
        async processDocument() {
          if (!this.documentFile || !this.movementDirection) return
          this.processing = true
          try {
            const formData = new FormData()
            formData.append('file', this.documentFile)
            formData.append('direction', this.movementDirection)
            const response = await this.$store.dispatch('movement/iaCreated', formData)
            if (response.data.status === 'OK') {
              ...
              this.$q.notify({
                type: 'negative',
                message: response.data.msg || 'Error al procesar el documento'
              })
            }
          } catch (error) {
            this.$q.notify({
        ...
        populateForm(data) {
  // Map movementDirection to the format expected by SelectInput
  const directionOption = this.directionOptions.find(opt => opt.value === this.movementDirection);
  // nombre del cliente
  this.movement.client_name = data.client_name || '';

  this.movement = {
    direction: directionOption ? { label: directionOption.label, value: directionOption.value } : null,
    reason_type: { label: data.reason_type.name, value: data.reason_type.id },
    supplier: data.supplier ? { label: data.supplier.name, value: data.supplier.id } : null,
    client_name: data.client_name || '',
    notes: data.notes || ''
  };
  this.movementDetails = data.details.map(detail => ({
    primary_product: detail.primary_product.id,
    product_color: detail.product_color ? detail.product_color.id : null,
    quantity: detail.quantity,
    unit_price: detail.unit_price,
    availableColors: detail.product_color
      ? [{ label: detail.product_color.name, value: detail.product_color.id }]
      : [],
    errors: {
      primary_product: '',
      product_color: '',
  ...
  this.primaryProducts = this.primaryProducts.length
    ? this.primaryProducts
    : data.details.map(detail => ({
        label: detail.primary_product.name,
        value: detail.primary_product.id
      }));
  this.suppliers = this.suppliers.length
    ? this.suppliers
    : data.supplier
    ? [{ label: data.supplier.name, value: data.supplier.id }]
    : [];
  this.reasonTypes = this.reasonTypes.length
    ? this.reasonTypes
    : [{ label: data.reason_type.name, value: data.reason_type.id }];
},
        ...
        async loadData() {
          ...
          try {
            ...
            if (!this.primaryProducts.length) {
        ...
        onRejected(rejectedEntries) {
          this.$q.notify({
            type: 'negative',
            message: `Archivos no válidos: ${rejectedEntries.map(e => e.file.name).join(', ')}`
          })
        }
      },
      mounted() {
