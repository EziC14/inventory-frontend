...
import { getSupplierId } from 'src/store/provedores/actions';
...
export default {
  ...
  computed: {
    ...
    verifyFirstStep() {
      // Solo el nombre del proveedor es obligatorio
  ...
  methods: {
    ...
    getSupplierId() {
      return this.$store.dispatch("provedores/getSupplierId", this.supplier.id , { root: true })
        .then((response) => {
          this.supplier = response.data.data;

          this.selectedProducts = response.data.data.products.map((o) => {
            return {
              ...o,
              selected: false,
            };
    ...
    getPrimaryProduct() {
      return this.$store.dispatch("general/getPrimaryProduct", { root: true })
        .then((response) => {
          this.availableProducts = response.data.data.map((o) => ({ ...o, selected: false}));

          this.filterById("selectedProducts", "availableProducts", true);
        })
        .finally(() => {
          this.$q.loading.hide();
        });
    ...
    onEdit() {
      ...
      this.$store.dispatch("provedores/updatedSupplierId", payload, { root: true })
        .then((response) => {
          if (response.data.status === "OK") {
            this.alertSuccess(response.data.msg || "Proveedor modificado con éxito");
            this.goTo('/provider');
          }
        })
        .catch((error) => {
          this.alertError(error.response?.data?.msg || "Error al modificado el proveedor");
        })
        .finally(() => {
          this.isLoading = false;
        });
